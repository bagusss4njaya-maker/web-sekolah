...
<div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Nilai</h2>
        <a href="{{ route('admin.grades.index') }}" class="text-gray-600 hover:text-gray-800">Kembali</a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.grades.update', $grade) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="student_id" name="student_id" required>
                    ...
                    @foreach($students as $student)
                        <option value="{{ $student->id }}" {{ old('student_id', $grade->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }} ({{ $student->nis }})</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="teacher_id">Guru Penilai</label>
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="teacher_id" name="teacher_id" required>
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $grade->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }} ({{ $teacher->username }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="task_score" type="number" min="0" max="100" name="task_score" value="{{ old('task_score', $grade->task_score) }}" required>
                ...
                <div>
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="midterm_score" type="number" min="0" max="100" name="midterm_score" value="{{ old('midterm_score', $grade->midterm_score) }}" required>
                ...
                <div>
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="final_score" type="number" min="0" max="100" name="final_score" value="{{ old('final_score', $grade->final_score) }}" required>
